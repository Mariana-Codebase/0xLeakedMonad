...
Start-Process -NoNewWindow -FilePath "cmd.exe" -ArgumentList "/c pnpm dev:api" -WorkingDirectory $root -RedirectStandardOutput "$root\.logs-api.txt" -RedirectStandardError "$root\.logs-api-err.txt"

Start-Sleep -Milliseconds 500

Write-Host "  [2/4] Breach Service (port 4101)" -ForegroundColor Yellow
Start-Process -NoNewWindow -FilePath "cmd.exe" -ArgumentList "/c pnpm dev:breach" -WorkingDirectory $root -RedirectStandardOutput "$root\.logs-breach.txt" -RedirectStandardError "$root\.logs-breach-err.txt"

Start-Sleep -Milliseconds 500

Write-Host "  [3/4] Analyzer Service (port 4102)" -ForegroundColor Yellow
Start-Process -NoNewWindow -FilePath "cmd.exe" -ArgumentList "/c pnpm dev:analyzer" -WorkingDirectory $root -RedirectStandardOutput "$root\.logs-analyzer.txt" -RedirectStandardError "$root\.logs-analyzer-err.txt"

Start-Sleep -Milliseconds 500

Write-Host "  [4/4] Web Frontend (port 3000)" -ForegroundColor Yellow
Start-Process -NoNewWindow -FilePath "cmd.exe" -ArgumentList "/c pnpm dev:web" -WorkingDirectory $root -RedirectStandardOutput "$root\.logs-web.txt" -RedirectStandardError "$root\.logs-web-err.txt"
